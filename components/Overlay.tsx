...
const Overlay = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    setMessages((prevMessages) => [...prevMessages, { id: prevMessages.length + 1, username: 'user', text: userMessage }]);
    setInputValue(""); // Clear input field
    setIsTyping(true);

    try {
      // Retrieve the token (example using Firebase)
      const token = await auth.currentUser?.getIdToken(); // Use the Firebase Auth instance
      // Optionally, check if token is available
      if (token) {
      ...
      setMessages((prevMessages) => [...prevMessages, { id: prevMessages.length + 2, username: 'assistant', text: reply }]);
    } catch (error) {
      if (error.response) {
        ...
        if (statusCode === 401) {
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { id: prevMessages.length + 2, username: 'assistant', text: "Missing the correct authorization headers" },
          ]);
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { id: prevMessages.length + 2, username: 'assistant', text: "Invalid token." },
          ]);
        } else {
          // Other errors
          console.error('Error calling FastAPI backend:', error.response.data || error.message);
          setMessages((prevMessages) => [
            ...prevMessages,
            { id: prevMessages.length + 2, username: 'assistant', text: "An error occurred. Please try again." },
          ]);
        }
      } else {
        // Handle cases where error.response is not available (e.g., network issues)
        console.error('Error with the network or request:', error.message);
        setMessages((prevMessages) => [
          ...prevMessages,
          { id: prevMessages.length + 2, username: 'assistant', text: "There was an issue with the network. Please check your connection and try again." },
        ]);
      }
